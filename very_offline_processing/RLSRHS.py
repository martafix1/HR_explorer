...
def rlsrhs_filter(r, t, f_est, harmonics_to_suppress=[2, 3, 4, 5]):
    """
    ...
    for i in range(N):
        ...
        for k in harmonics_to_suppress:
            # We construct sine and cosine references for each harmonic
            X[idx, 0] = np.cos(2 * np.pi * k * f_est * t[i])
            X[idx+1, 0] = np.sin(2 * np.pi * k * f_est * t[i])
            idx += 2
...
fs = 35.0  # 50ms = 20Hz sampling rate
...
e = rlsrhs_filter(r, t, f_est, harmonics_to_suppress=[2, 3, 4, 5])
