...
if __name__ == "__main__":
    ...
    result = extract_vital_patterns(sig_A_detrend, FS_HZ)
    breath = result.by_name("breath")
    heart = result.by_name("heart")

    breath.instantaneous_amplitude
    breath.instantaneous_frequency_hz

    fig, ax = plt.subplots(3, 1 ) #, figsize=(7.2, 3.2))
    ax[0].plot(t_A, sig_A_detrend, label="A")
    ax[0].plot(t_A, breath.instantaneous_amplitude, label="A Breath ampl")
    ax[0].plot(t_A, heart.instantaneous_amplitude, label="A HR ampl")
    ax[0].set_xlabel("time [s]")
    ax[0].set_ylabel("displacement [mm]")
    ...
    ax[1].plot(t_A, breath.instantaneous_frequency_hz, label="BR A")
    # ax[1].plot(t_A, sig_A_inst_A, label="inst. freq")
    ax[1].set_xlabel("time [s]")
    ax[1].set_ylabel("freq [Hz]")
    ax[1].set_title("Breath rate")
    ax[1].legend()
    # ax[1].set_yscale('log')

    ax[2].plot(t_A, heart.instantaneous_frequency_hz, label="HR A")
